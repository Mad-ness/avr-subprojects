...
struct protocol_t_ {
    int pulse_length;
    struct pulse_t_ sync_factor;
    struct pulse_t_ zero;
    struct pulse_t_ one;
    bool inverted;
};
...
inline uint16_t RCTSwitch_getValue_() { return nReceivedValue; }
inline bool RCTSwitch_available_() { nReceivedValue != 0; }
inline void RCTSwitch_reset_() { nReceivedValue = 0; }

bool RCTSwitch_receiveProtocol_(const int p, const int changeCount) {
...
void loop(void) {
    ...
    if (millis() % 100 == 0) {
        ...
        if ( RCTSwitch_available() ) {
            ...
            //if ( sender_id == unpack_senderId(recv_data) ) {
                ...
                if ( setup_minus )
                    t_value *= -1;
                if ( recv_intpart <= t_value ) {
                    has_threshold_crossed = true;
                    led_on();
                } else {
    ...
    //    display_number(debug_num);
    }
}

void RCTSwitch_setup_() {
    GIMSK |= ( 1 << PCIE ); //|(1<<INT0);
    PCMSK |= ( 1 << PCINT0 );
    MCUCR |= ( 1 << ISC00 );
}
